...
$str = "INSERT INTO log SET `datetime` = '".NOW()."'";
$SQLObj0->RunSQL($str);
...
if((gmdate("H")+GMHOUR).gmdate("i") >= '1750' && (gmdate("H")+GMHOUR).gmdate("i") <= '1850')
{
...
$today = "2008-".sprintf("%02d",$mm)."-".sprintf("%02d",$dd);
...
$str = "SELECT * FROM myteam_data WHERE lastlogin>='".$lastlogin."' AND ready=1";
$SQLObj0->RunSQL($str);
...
for($i=0;$i<$allteamcount;$i++)
{
	...
	if($SQLObj0->LinkNext())
	{
		...
		//取得trade(0611)
		$trade_1 = GetTrade1($SQLObj0,$teamid);
		$trade_2 = GetTrade2($SQLObj0,$teamid);
		//檢查今日是否已經變動過了
		$mktime = gmmktime(19,0,0,$mm,$dd,$yy);
		$today = gmdate("Y-m-d",$mktime);
...
//是否已經更新過了
$str = "INSERT INTO log_pricemove SET `datetime` = '".NOW()."',teamcount=".$teamcount;
$SQLObj0->RunSQL($str);
