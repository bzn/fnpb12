...
define("HANJI_DOUBLE_1", "２");
define("HANJI_TRIPLE_1", "３");
...
//日本時間
$JPDateTime 	= GetJPTime();
$JPDateTimeY	= substr($JPDateTime, 0, 4);
$JPDateTimeM	= sprintf("%02d", substr($JPDateTime, 5, 2));
$JPDateTimeD	= sprintf("%02d", substr($JPDateTime, 8, 2));

var_dump((object)$array);

//Yahoo比賽日期
//$iGameDate 		= sprintf("%4d%02d%02d", $JPDateTimeY, $JPDateTimeM, $JPDateTimeD);
//日本時間
//$iGameDateTime	= sprintf("%4d-%02d-%02d",$JPDateTimeY, $JPDateTimeM, $JPDateTimeD);

//Yahoo比賽日期(測試用資料)
//$iGameDate 		= '20110412';
//日本時間(測試用資料)
//$iGameDateTime	= '2011-04-12';

if($_REQUEST['GameDate'])
{
	$iGameDate = $_REQUEST['GameDate'];
}

if($_REQUEST['GameDateTime'])
{
	$arrGameDateTime = explode("-", $_REQUEST['GameDateTime']);
	
	$iGameDateTime 	= sprintf("%4d-%02d-%02d", $arrGameDateTime[0], $arrGameDateTime[1], $arrGameDateTime[2]);
	$iGameDate		= sprintf("%4d%02d%02d", $arrGameDateTime[0], $arrGameDateTime[1], $arrGameDateTime[2]);
}

//初始本日投打者記錄(每日購買已經有填入不再填)
//utilGameDataManager::sharedDataManager()->initDailyRecord($iGameDateTime);

if(empty($arrGameDateTime[0]) || empty($arrGameDateTime[1]) || empty($arrGameDateTime[0]))
{
	exit();
}

if(empty($iGameDate) || empty($iGameDateTime))
{
	exit();
}
...
for ($iGame = 1 ; $iGame <= 6 ; $iGame++)
{
	...
	if($urlToParse = sprintf("http://baseball.yahoo.co.jp/npb/game/%d/top", $gameId))
	{
		...
		if($bIsGameOver)
		{
			...
			//解析打者成績
			$pPlayerList = "/<tr( class=\"al\"|)><td>.*?<\/td><td class=\"pn\".*?<\/tr>/";
			$arrMatchList = array();
			...
			foreach ($arrMatchList[0] as $strMatchData)
			{
				...
				if($attrData[0] != HANJI_PITCHER_1 && $attrData[0] != HANJI_PITCHER_2)
				{
					...
					//盜壘
					$objDailyRecordH->sb	= $attrData[10];
					//本打
			...
			foreach ($arrMatchList[0] as $strMatchData)
			{
				...
				//whip
				if($objDailyRecordP->ip > 0 || $objDailyRecordP->ip2 > 0)
				{
					$objDailyRecordP->whip	= ($objDailyRecordP->h + $objDailyRecordP->bb) / ($objDailyRecordP->ip + ($objDailyRecordP->ip2 / 3)); 
				}
				// 計算積分 寫入DB
